...
exports.exposedMiniget = (url, options = {}, requestOptionsOverwrite) => {
  const requestOptions = requestOptionsOverwrite || options.requestOptions;
  const req = miniget(
    options.rewriteUrl ? options.rewriteUrl(url) : url,

    options.rewriteRequestOptions
      ? options.rewriteRequestOptions(requestOptions, url)
      : requestOptions
  );
